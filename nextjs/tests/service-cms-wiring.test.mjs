...
test('service collections preserve authored empty arrays and CMS images keep configurable crop data', () => {
  ...
  assert.match(cmsImage, /resolveSource\(imageValue, width, height, fit, quality\)/)
  assert.match(cmsImage, /fit = 'crop'/)
  assert.match(cmsImage, /fit === 'crop' \? value : value\.asset/)
  assert.match(cmsImage, /height && fit === 'crop'/)
  assert.match(cmsImage, /fit !== 'crop'/)
  assert.match(cmsImage, /\.fit\(fit\)/)
  assert.match(cmsImage, /objectFit: 'contain'/)
  assert.match(cmsImage, /transform: style\?\.transform \?\? 'none'/)
  assert.match(cmsImage, /fill \? \{fill: true\}/)
  assert.match(cmsImage, /unoptimized=\{resolvedSrc\.startsWith\('https:\/\/cdn\.sanity\.io\/'\)\}/)
  assert.match(cmsImage, /placeholder=\{blurDataURL \? 'blur' : 'empty'\}/)
