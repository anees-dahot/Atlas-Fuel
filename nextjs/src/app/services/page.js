...
export const metadata = {
  title: 'Services | Atlas Fuel Australia',
  description: 'Comprehensive fuel services across Australia - mining, marine, agriculture, transportation, and more.',
}
...
export default async function ServicesPage() {
  ...
  const hero = {
    ...
    heroImageAlt: sanity?.heroSection?.heroImageUrlAlt || fallbackHero.heroImageAlt,
  ...
  const services = {
    ...
    services: sanity?.servicesSection?.services?.length ? sanity.servicesSection.services.map((s, i) => ({
      _key: s._key,
      ...
      imageAlt: s.imageUrlAlt || s.title || fallbackServices.services[i]?.title || '',
  ...
  const stats = {
    ...
    stats: sanity?.statsSection?.stats?.length ? sanity.statsSection.stats.map((s, i) => ({
  ...
  const timeline = {
    ...
    steps: sanity?.timelineSection?.steps?.length ? sanity.timelineSection.steps.map((s, i) => ({
  ...
  const additionalSectionsData = {
    ...
    bunkerRefueling: {
      ...
      ctaText: sanity?.bunkerRefuelingSection?.ctaText || 'Contact Us for Marine Fuel',
      ...
      imageAlt: sanity?.bunkerRefuelingSection?.imageUrlAlt || additionalSections.bunkerRefueling.imageAlt,
    },
    ownStation: {
      ...
      imageAlt: sanity?.ownStationSection?.imageUrlAlt || additionalSections.ownStation.imageAlt,
    },
    sectorsCover: {
      ...
      sectors: sanity?.sectorsCoverSection?.sectors?.length ? sanity.sectorsCoverSection.sectors.map((s, i) => ({
        _key: s._key,
        ...
        imageAlt: s.imageUrlAlt || s.name || additionalSections.sectorsCover.sectors[i]?.name || '',
  ...
  const industriesGrid = {
    ...
    industries: sanity?.industriesGridSection?.industries?.length
          ...
          <div className="max-w-[1440px] mx-auto px-8 lg:px-12">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <a href="/contact" className={`inline-flex items-center gap-2 ${additionalSectionsData.bunkerRefueling.ctaTextColor} font-semibold hover:underline`} style={bunkerRefuelingCTAStyle}>
              ...
              <div className="relative h-[400px] overflow-hidden">
                <img src={additionalSectionsData.bunkerRefueling.imageUrl} alt={additionalSectionsData.bunkerRefueling.imageAlt} className="w-full h-full object-cover" />
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-[1440px] mx-auto px-8 lg:px-12">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div className="relative h-[400px] overflow-hidden lg:order-1">
                <img src={additionalSectionsData.ownStation.imageUrl} alt={additionalSectionsData.ownStation.imageAlt} className="w-full h-full object-cover" />
              </div>
