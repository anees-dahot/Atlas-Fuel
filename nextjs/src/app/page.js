...
const homeQuery = groq`*[_id == "homePage"][0]{
  ...
  featureBoxesSection {
    cards[]{
      ...
      link, "imageUrl": coalesce(image.asset->url, imageUrl), "imageAlt": image.alt
    }
  },
  whatWeDoSection {
    ...
    sectors[]{
      ...
      description, descriptionColor, descriptionSize, descriptionBorderEnabled, descriptionBorderColor, descriptionBorderWidth, descriptionShadowColor,
      fullDescription, "imageUrl": image.asset->url, "imageAlt": image.alt, href,
  ...
  visionSection {
    ...
    "videoImageUrl": videoImage.asset->url, "videoImageAlt": videoImage.alt, videoLabel, videoLabelColor, videoLabelSize, videoLabelBorderEnabled, videoLabelBorderColor, videoLabelBorderWidth, videoLabelShadowColor, videoDescription
  },
  communitySection {
    sectionTag, sectionTagColor, sectionTagSize, sectionTagBorderEnabled, sectionTagBorderColor, sectionTagBorderWidth, sectionTagShadowColor,
  ...
  aboutSection {
    ...
    keyPoints, "imageLeftUrl": imageLeft.asset->url, "imageLeftAlt": imageLeft.alt, "imageRightUrl": imageRight.asset->url, "imageRightAlt": imageRight.alt,
    ctaText, ctaTextColor, ctaTextSize, ctaTextBorderEnabled, ctaTextBorderColor, ctaTextBorderWidth, ctaTextShadowColor, ctaLink,
    stats[]{ _key, value, valueColor, valueSize, valueBorderEnabled, valueBorderColor, valueBorderWidth, valueShadowColor, label, labelColor, labelSize, labelBorderEnabled, labelBorderColor, labelBorderWidth, labelShadowColor }
  },
  certificationsSection {
    ...
    heading, headingColor, headingSize, headingBorderEnabled, headingBorderColor, headingBorderWidth, headingShadowColor,
    description, descriptionColor, descriptionSize, descriptionBorderEnabled, descriptionBorderColor, descriptionBorderWidth, descriptionShadowColor,
    certificateImages[]{ _key, "url": asset->url, alt },
    certifications[]{ _key, type, title, titleColor, titleSize, titleBorderEnabled, titleBorderColor, titleBorderWidth, titleShadowColor, description, descriptionColor, descriptionSize, descriptionBorderEnabled, descriptionBorderColor, descriptionBorderWidth, descriptionShadowColor, "certificateImageUrl": certificateImage.asset->url, "certificateImageAlt": certificateImage.alt }
  },
  peopleSection {
    sectionTag, sectionTagColor, sectionTagSize, sectionTagBorderEnabled, sectionTagBorderColor, sectionTagBorderWidth, sectionTagShadowColor,
    heading, headingColor, headingSize, headingBorderEnabled, headingBorderColor, headingBorderWidth, headingShadowColor,
    ...
    "cardImageUrl": cardImageUrl.asset->url,
    cardBadge,
...
const newsQuery = groq`*[_type == "newsPost"] | order(publishedAt desc)[0...3]{ title, "slug": slug.current, publishedAt, author, excerpt, "imageUrl": mainImage.asset->url }`;

const sizeMap = { '1': '12px', '2': '16px', '3': '20px', '4': '24px', '5': '32px', '6': '48px', '7': '70px' };

export default async function Home() {
  const homeCache = process.env.CMS_ROUNDTRIP_TEST === "1"
    ? { cache: "no-store" }
    : { next: { revalidate: 60, tags: ["homePage"] } };
  const newsCache = process.env.CMS_ROUNDTRIP_TEST === "1"
    ? { cache: "no-store" }
    : { next: { revalidate: 60, tags: ["newsPost"] } };
  const [sanity, newsPosts, globalSettings] = await Promise.all([
    client.fetch(homeQuery, {}, homeCache).catch((err) => { console.error("[Sanity] homeQuery failed:", err.message); return null; }),
    client.fetch(newsQuery, {}, newsCache).catch((err) => { console.error("[Sanity] newsQuery failed:", err.message); return []; }),
  ...
  const hero = { ...fbHero,
    ...
    title: s?.heroSection?.titleLine1 ? [
    ...
    quickLinks: s?.heroSection?.quickLinks?.length
      ...
      : fbHero.quickLinks,
    heroStats: s?.heroSection?.heroStats?.length
      ? s.heroSection.heroStats
      : fbHero.heroStats,
  };

  const featureCards = s?.featureBoxesSection?.cards?.length
  ...
  const sectors = s?.whatWeDoSection?.sectors?.length
    ? s.whatWeDoSection.sectors.map((sec,i) => ({ 
        ...
        stats: sec.stats?.length ? sec.stats : fbSectors[i]?.stats||[] 
      }))
    : fbSectors;
  const whatWeDo = {
