...
export default function Footer({ data = {}, footerNavigation, siteSettings }) {
  // Use siteSettings for contact info
  ...
  // Use footerNavigation for links with fallbacks
  const companyHeading = footerNavigation?.companySectionHeading || 'Company'
  const companyLinks = footerNavigation?.companyLinks && footerNavigation.companyLinks.length > 0
    ? footerNavigation.companyLinks
    : defaultCompanyLinks

  const servicesHeading = footerNavigation?.servicesSectionHeading || 'Services'
  const servicesLinks = footerNavigation?.servicesLinks && footerNavigation.servicesLinks.length > 0
    ? footerNavigation.servicesLinks
    : defaultServicesLinks

  const supportHeading = footerNavigation?.supportSectionHeading || 'Support'
  const supportLinks = footerNavigation?.supportLinks && footerNavigation.supportLinks.length > 0
    ? footerNavigation.supportLinks
    : defaultSupportLinks

  const legalLinks = footerNavigation?.legalLinks && footerNavigation.legalLinks.length > 0
      ...
      <div className="max-w-[1440px] mx-auto px-8 lg:px-12 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-2">
            <div className="mb-6">
              <Image src={siteSettings?.logoUrl || '/images/logo.png'} alt={siteSettings?.siteName || 'Atlas Fuel'} width={140} height={56} className="h-14 w-auto" />
            </div>
            <p className="text-gray-600 mb-6 max-w-sm leading-relaxed">{description}</p>
            <div className="space-y-3">
              <a href={`tel:${phone.replace(/\s/g, '')}`} className="flex items-center gap-3 text-gray-600 hover:text-primary transition-colors">
              ...
              <a href={`mailto:${email}`} className="flex items-center gap-3 text-gray-600 hover:text-primary transition-colors">
            ...
            <div className="flex items-center gap-3 mt-6">
              <a href={fb} className="w-10 h-10 bg-sand border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-primary hover:text-white hover:border-primary transition-all duration-300" aria-label="Facebook">
              ...
              <a href={ig} className="w-10 h-10 bg-sand border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-primary hover:text-white hover:border-primary transition-all duration-300" aria-label="Instagram">
              ...
              <a href={li} className="w-10 h-10 bg-sand border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-primary hover:text-white hover:border-primary transition-all duration-300" aria-label="LinkedIn">
