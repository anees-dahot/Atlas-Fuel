...
export default function AboutValues({ data = {} }) {
  ...
  const eyebrow = data.eyebrow ?? 'What We Offer'
  ...
  const bodyBorderColor = data.whatWeOfferBorderColor ?? 'var(--cms-text)'
        ...
        <div className="grid lg:grid-cols-12 gap-16 lg:gap-24">
          ...
          <div className="aval-list lg:col-span-7 border-t border-gray-200">
            {values.map((v, i) => {
              ...
              const contentBorderColor = v.contentBorderColor ?? 'var(--cms-text)'
              ...
              return (
                <div key={v._key ?? `${v.title}-${i}`} className="aval-item group relative flex gap-8 py-10 border-b border-gray-200 hover:border-primary/30 transition-colors duration-500 overflow-hidden">
                  {/* Number */}
